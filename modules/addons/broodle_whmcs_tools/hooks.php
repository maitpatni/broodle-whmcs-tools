...
function broodle_tools_get_emails($serviceId)
{
    ...
    // Resolve credentials — access hash may be a plaintext API token or encrypted
    $accessHash = '';
    $password = '';

    if (!empty($server->accesshash)) {
        $raw = trim($server->accesshash);
        // API tokens are short alphanumeric strings (< 64 chars, no special chars)
        // Encrypted values contain special chars like /, +, = and are typically longer
        if (preg_match('/^[A-Za-z0-9]{10,64}$/', $raw)) {
            // Looks like a plaintext API token — use directly
            $accessHash = $raw;
        } else {
            // Likely encrypted — try to decrypt
            $accessHash = trim(decrypt($raw));
            // If decrypt returned empty or garbage, clear it
    ...
    // Use cPanel UAPI via WHM to list email accounts for the user
    // WHM API: cpanel call using UAPI Email::list_pops
    ...
    // Try parsing UAPI response
    if ($httpCode === 200 && $response) {
        $json = json_decode($response, true);
        if (!empty($json)) {
            // UAPI via WHM wrapper: result.data[]
            $pops = [];
            if (isset($json['result']['data'])) {
                $pops = $json['result']['data'];
            } elseif (isset($json['cpanelresult']['data'])) {
                $pops = $json['cpanelresult']['data'];
            ...
            foreach ($pops as $entry) {
                ...
                if (is_string($entry)) {
                    ...
                    $email = $entry['email'] ?? ($entry['login'] ?? ($entry['user'] ?? ''));
                }
    ...
    // Fallback: try WHM API 1 list_pops_for if UAPI returned nothing
    if (empty($emails)) {
        $url2 = "{$protocol}://{$hostname}:{$port}/json-api/list_pops_for"
              . "?api.version=1"
        ...
        if (!empty($headers)) {
            ...
            curl_setopt($ch2, CURLOPT_USERPWD, "{$serverUser}:{$password}");
        }
        ...
        if ($httpCode2 === 200 && $response2) {
            ...
            if (!empty($json2)) {
                $pops2 = [];
                if (isset($json2['data']['pops'])) {
                    $pops2 = $json2['data']['pops'];
                } elseif (isset($json2['pops'])) {
                    $pops2 = $json2['pops'];
                }

                foreach ($pops2 as $entry) {
                    $email = '';
                    if (is_string($entry)) {
                        $email = $entry;
                    } elseif (is_array($entry)) {
    ...
    // Final fallback: try UAPI Email::list_pops_with_disk directly
    if (empty($emails)) {
        ...
        if (!empty($headers)) {
            ...
            curl_setopt($ch3, CURLOPT_USERPWD, "{$serverUser}:{$password}");
        }
        ...
        if ($httpCode3 === 200 && $response3) {
            $json3 = json_decode($response3, true);
            if (!empty($json3)) {
                $pops3 = [];
                if (isset($json3['result']['data'])) {
                    $pops3 = $json3['result']['data'];
                } elseif (isset($json3['cpanelresult']['data'])) {
                    $pops3 = $json3['cpanelresult']['data'];
                } elseif (isset($json3['data'])) {
                    $pops3 = $json3['data'];
                }

                foreach ($pops3 as $entry) {
                    $email = '';
                    if (is_string($entry)) {
                        $email = $entry;
                    } elseif (is_array($entry)) {
...
function broodle_tools_build_email_output($emails, $serviceId)
{
    ...
    $emailRows = '';
    if ($count === 0) {
        $emailRows = '<div class="bem-empty">'
            . '<svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg>'
            . '<span>No email accounts found</span></div>';
    } else {
        foreach ($emails as $email) {
            ...
            $emailRows .= '<div class="bem-row" data-email="' . $e . '">'
                ...
                . '<button type="button" class="bem-btn bem-btn-login" data-email="' . $e . '" title="Open Webmail">'
                . '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4"/><polyline points="10 17 15 12 10 7"/><line x1="15" y1="12" x2="3" y2="12"/></svg>'
                . '<span>Login</span></button>'
                . '<button type="button" class="bem-btn bem-btn-pass" data-email="' . $e . '" title="Change Password">'
                . '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>'
                . '<span>Password</span></button>'
                . '<button type="button" class="bem-btn bem-btn-del" data-email="' . $e . '" title="Delete">'
...
<div class="bem-overlay" id="bemCreateModal" style="display:none">
  <div class="bem-modal">
    <div class="bem-modal-head">
      <h5>Create Email Account</h5>
      <button type="button" class="bem-modal-close" data-close>&times;</button>
    </div>
    <div class="bem-modal-body">
      <div class="bem-field">
        <label>Email Address</label>
        <div class="bem-input-group">
          <input type="text" id="bemNewUser" placeholder="username" autocomplete="off">
          <span class="bem-at">@</span>
          <select id="bemNewDomain"><option>Loading...</option></select>
        </div>
      </div>
      <div class="bem-field">
        <label>Password</label>
        <div class="bem-pass-wrap">
          <input type="password" id="bemNewPass" placeholder="Strong password" autocomplete="new-password">
          <button type="button" class="bem-pass-toggle" data-toggle-pass="bemNewPass">
      ...
      <div class="bem-field">
        ...
        <input type="number" id="bemNewQuota" value="250" min="1">
      </div>
      <div class="bem-modal-msg" id="bemCreateMsg"></div>
    </div>
    <div class="bem-modal-foot">
      <button type="button" class="bem-mbtn bem-mbtn-cancel" data-close>Cancel</button>
      <button type="button" class="bem-mbtn bem-mbtn-primary" id="bemCreateSubmit">Create Account</button>
    </div>
  </div>
</div>
...
<div class="bem-overlay" id="bemPassModal" style="display:none">
  <div class="bem-modal">
    ...
    <div class="bem-modal-body">
      <div class="bem-field">
        <label>Email</label>
        <input type="text" id="bemPassEmail" readonly>
      </div>
      <div class="bem-field">
        <label>New Password</label>
        <div class="bem-pass-wrap">
          <input type="password" id="bemPassNew" placeholder="New password" autocomplete="new-password">
          <button type="button" class="bem-pass-toggle" data-toggle-pass="bemPassNew">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
          </button>
        </div>
      </div>
    ...
    <div class="bem-modal-foot">
      <button type="button" class="bem-mbtn bem-mbtn-cancel" data-close>Cancel</button>
      <button type="button" class="bem-mbtn bem-mbtn-primary" id="bemPassSubmit">Update Password</button>
    </div>
  </div>
</div>
...
<div class="bem-overlay" id="bemDelModal" style="display:none">
  <div class="bem-modal bem-modal-sm">
    <div class="bem-modal-head">
      <h5>Delete Email Account</h5>
      <button type="button" class="bem-modal-close" data-close>&times;</button>
    </div>
    <div class="bem-modal-body" style="text-align:center">
      <div style="margin:8px 0 16px">
        <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="1.5" style="margin:0 auto;display:block"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>
      </div>
    ...
    <div class="bem-modal-foot">
      <button type="button" class="bem-mbtn bem-mbtn-cancel" data-close>Cancel</button>
      <button type="button" class="bem-mbtn bem-mbtn-danger" id="bemDelSubmit">Delete</button>
    </div>
  </div>
</div>';
}
