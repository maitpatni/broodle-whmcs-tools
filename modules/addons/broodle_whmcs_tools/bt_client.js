...
'.bwp-overview-hero{display:grid;grid-template-columns:340px 1fr;gap:20px}@media(max-width:700px){.bwp-overview-hero{grid-template-columns:1fr}}',
...
'.bwp-theme-screenshot{height:120px;background:var(--input-bg,#f3f4f6);overflow:hidden;position:relative}.bwp-theme-screenshot img{width:100%;height:100%;object-fit:cover}.bwp-theme-placeholder{width:100%;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;background:var(--input-bg,#f3f4f6)}',
...
/* Tools tab */
'.bwp-tools-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}@media(max-width:600px){.bwp-tools-grid{grid-template-columns:1fr}}',
'.bwp-tool-card{display:flex;align-items:center;gap:12px;padding:14px 16px;background:var(--input-bg,#f8fafc);border:1px solid var(--border-color,#e5e7eb);border-radius:10px;transition:border-color .15s}',
'.bwp-tool-card:hover{border-color:rgba(10,94,211,.2)}',
'.bwp-tool-icon{width:36px;height:36px;border-radius:9px;display:flex;align-items:center;justify-content:center;flex-shrink:0}',
'.bwp-tool-info{flex:1;min-width:0}.bwp-tool-name{margin:0;font-size:13px;font-weight:600;color:var(--heading-color,#111827)}.bwp-tool-desc{margin:2px 0 0;font-size:11px;color:var(--text-muted,#6b7280)}',
'.bwp-tool-toggle{flex-shrink:0}',
'.bwp-toggle-btn{position:relative;width:44px;height:24px;border-radius:12px;border:none;background:#d1d5db;cursor:pointer;transition:background .2s;padding:0}',
'.bwp-toggle-btn.on{background:#059669}',
'.bwp-toggle-knob{position:absolute;top:2px;left:2px;width:20px;height:20px;border-radius:50%;background:#fff;box-shadow:0 1px 3px rgba(0,0,0,.2);transition:transform .2s}',
'.bwp-toggle-btn.on .bwp-toggle-knob{transform:translateX(20px)}',
'.bwp-toggle-btn:disabled{opacity:.5;cursor:not-allowed}',
'.bwp-theme-vulnerable{border-color:#ef4444;box-shadow:0 0 0 1px rgba(239,68,68,.2)}',
    ].join('\n');
    document.head.appendChild(s);
}
...
window.btOpenCpanelPage=function(page,el){
    ...
    post({action:'get_cpanel_sso_url',page:page||''},function(r){
        ...
        if(r.success&&r.url){
            ...
            fmToast(r.message||'Could not create cPanel session','error');
            /* Fallback: try WHMCS native SSO */
            window.open('clientarea.php?action=productdetails&id='+C.serviceId+'&dosinglesignon=1','_blank');
        }
    });
};
...
function buildEmailPageInto(container){
    ...
    var qa=buildQuickAccess([
        {icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg>',label:'Email Accounts',page:'email'},
        {icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22 6 12 13 2 6"/></svg>',label:'Forwarders',page:'forwarders'},
        {icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg>',label:'Email Routing',page:'emailrouting'},
        {icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>',label:'Spam Filters',page:'spamfilters'}
    ]);
    /* Tab navigation */
    var tabHtml='<div class="bt-email-tabs" id="bt-email-tabs">'
    +'<button class="bt-email-tab active" data-etab="accounts"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg>Email Accounts</button>'
    +'<button class="bt-email-tab" data-etab="connect"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>Connect &amp; Apps</button>'
    +'</div>';
    container.innerHTML=qa+tabHtml+'<div id="bt-email-tab-accounts"></div><div id="bt-email-tab-connect" style="display:none"></div>';
    /* Tab switching */
    container.querySelectorAll(".bt-email-tab").forEach(function(tab){
        tab.addEventListener("click",function(){
            container.querySelectorAll(".bt-email-tab").forEach(function(t){t.classList.remove("active");});
            tab.classList.add("active");
            var target=tab.getAttribute("data-etab");
            $("bt-email-tab-accounts").style.display=target==="accounts"?"":"none";
            $("bt-email-tab-connect").style.display=target==="connect"?"":"none";
            if(target==="connect"&&!$("bt-email-tab-connect").dataset.loaded){
                $("bt-email-tab-connect").dataset.loaded="1";
                buildEmailConnectPane();
            }
        });
    });
    /* Build accounts pane into the tab container */
    var accountsPane=$("bt-email-tab-accounts");
    accountsPane.innerHTML='<div id="bt-pane-email"></div>';
    buildEmailPane();
}
...
function buildEmailConnectPane(){
    ...
    if(maEnabled){
        ...
        if(maPlay){
            html+='<a href="'+esc(maPlay)+'" target="_blank" rel="nofollow noopener" class="bt-email-app-store-btn android">'
        ...
        if(maApple){
            html+='<a href="'+esc(maApple)+'" target="_blank" rel="nofollow noopener" class="bt-email-app-store-btn ios">'
    ...
    +'<a class="bt-email-client-item" href="https://support.microsoft.com/en-us/office/add-an-email-account-to-outlook-for-windows-6e27792a-9267-4aa4-8bb6-c84ef146101b" target="_blank" rel="nofollow noopener"><div class="bt-email-client-icon" style="background:rgba(10,94,211,.08)"><img src="modules/addons/broodle_whmcs_tools/outlook-logo.png" width="28" height="28" alt="Outlook" style="object-fit:contain"></div><span>Outlook</span><small>IMAP / Port 993</small></a>'
    +'<a class="bt-email-client-item" href="https://support.google.com/mail/answer/6078445" target="_blank" rel="nofollow noopener"><div class="bt-email-client-icon" style="background:rgba(234,67,53,.08)"><img src="modules/addons/broodle_whmcs_tools/gmail-icon.png" width="28" height="28" alt="Gmail" style="object-fit:contain"></div><span>Gmail App</span><small>IMAP / Port 993</small></a>'
    +'<a class="bt-email-client-item" href="https://support.apple.com/en-us/102619" target="_blank" rel="nofollow noopener"><div class="bt-email-client-icon" style="background:rgba(0,0,0,.05)"><img src="modules/addons/broodle_whmcs_tools/apple-mail-logo.png" width="28" height="28" alt="Apple Mail" style="object-fit:contain"></div><span>Apple Mail</span><small>IMAP / Port 993</small></a>'
    +'<a class="bt-email-client-item" href="https://support.mozilla.org/en-US/kb/manual-account-configuration" target="_blank" rel="nofollow noopener"><div class="bt-email-client-icon" style="background:rgba(0,97,224,.08)"><img src="modules/addons/broodle_whmcs_tools/thunderbird-logo.png" width="28" height="28" alt="Thunderbird" style="object-fit:contain"></div><span>Thunderbird</span><small>IMAP / Port 993</small></a>'
    +'</div>'
    +'</div></div>';
    ...
    /* Bind copy-on-click for config values */
    pane.querySelectorAll(".bt-email-config-val.copyable").forEach(function(el){
        el.addEventListener("click",function(){
            var val=this.getAttribute("data-copy");
            if(navigator.clipboard&&navigator.clipboard.writeText){
                navigator.clipboard.writeText(val).then(function(){fmToast("Copied: "+val,true);});
            }else{
                var tmp=document.createElement("textarea");tmp.value=val;document.body.appendChild(tmp);tmp.select();document.execCommand("copy");document.body.removeChild(tmp);
                fmToast("Copied: "+val,true);
            }
        });
    });
}
...
function buildDomainPageInto(container){
    if(!container) return;
    var qa=buildQuickAccess([
        {icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10A15.3 15.3 0 0 1 12 2z"/></svg>',label:'Addon Domains',page:'addondomains'},
        {icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="16 3 21 3 21 8"/><line x1="4" y1="20" x2="21" y2="3"/></svg>',label:'Subdomains',page:'subdomains'},
        {icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/></svg>',label:'DNS Zone Editor',page:'dns'},
        {icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>',label:'Redirects',page:'redirects'},
        {icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>',label:'SSL/TLS Status',page:'sslstatus'}
    ]);
...
function buildDatabasePageInto(container){
    if(!container) return;
    var qa=buildQuickAccess([
        {icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3"/><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5"/></svg>',label:'MySQL Databases',page:'databases'},
        {icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>',label:'phpMyAdmin',page:'phpmyadmin'},
        {icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>',label:'MySQL Users',page:'databases'},
        {icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>',label:'Remote MySQL',page:'remotemysql'}
    ]);
...
function buildSSLPageInto(container){
    ...
    var qa=buildQuickAccess([
        {icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>',label:'SSL/TLS Status',page:'sslstatus'},
        {icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>',label:'SSL/TLS Manager',page:'ssl'},
        {icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/></svg>',label:'AutoSSL',page:'autossl'},
        {icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>',label:'Install SSL',page:'sslinstall'}
    ]);
...
function loadDatabases(){
    ...
    post({action:"list_databases"},function(r){
        ...
        list.querySelectorAll(".bt-row-btn.del[data-db]").forEach(function(b){b.addEventListener("click",function(){
            if(confirm("Delete database "+this.getAttribute("data-db")+"?")){var btn=this;btnLoad(btn,"");post({action:"delete_database",database:this.getAttribute("data-db")},function(r){btnDone(btn);if(r.success)loadDatabases();else fmToast(r.message||"Failed","error");});}
        });});
        list.querySelectorAll(".bt-row-btn.del[data-dbuser]").forEach(function(b){b.addEventListener("click",function(){
            if(confirm("Delete user "+this.getAttribute("data-dbuser")+"?")){var btn=this;btnLoad(btn,"");post({action:"delete_db_user",dbuser:this.getAttribute("data-dbuser")},function(r){btnDone(btn);if(r.success)loadDatabases();else fmToast(r.message||"Failed","error");});}
        });});
...
function bwpOpenDetail(id){currentWpInstance=null;for(var i=0;i<wpInstances.length;i++){if(wpInstances[i].id===id){currentWpInstance=wpInstances[i];break;}}if(!currentWpInstance) return;var wpPage=$("bt-wp-page");if(!wpPage) return;var siteUrl=currentWpInstance.site_url||"";var totalUpdates=(currentWpInstance.pluginUpdates||0)+(currentWpInstance.themeUpdates||0)+(currentWpInstance.availableUpdate?1:0);var html='<div class="bwp-inline-detail" id="bwp-inline-detail">';html+='<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:10px"><button type="button" class="bt-btn-outline bwp-back-btn" id="bwpInlineBack" style="padding:7px 14px;display:inline-flex;align-items:center;gap:6px"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6"/></svg> Back to Sites</button><div style="display:flex;gap:8px"><button type="button" class="bt-btn-add" id="bwpDetailLogin" style="padding:7px 16px"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4"/><polyline points="10 17 15 12 10 7"/><line x1="15" y1="12" x2="3" y2="12"/></svg> WP Admin</button><a href="'+esc(siteUrl)+'" target="_blank" class="bt-btn-outline" style="padding:7px 16px;display:inline-flex;align-items:center;gap:6px;text-decoration:none"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg> Visit Site</a></div></div>';html+='<div class="bt-card" style="overflow:visible;border-radius:14px">';html+='<div class="bwp-detail-head"><div class="bwp-card-wp-icon" style="width:36px;height:36px;border-radius:9px;background:rgba(33,117,208,.08);display:flex;align-items:center;justify-content:center;flex-shrink:0">'+wpSvg20+'</div><div style="flex:1;min-width:0"><h5 style="margin:0;font-size:16px;font-weight:700;color:var(--heading-color,#111827)">'+esc(currentWpInstance.displayTitle||currentWpInstance.domain)+'</h5><div style="display:flex;gap:6px;margin-top:4px;flex-wrap:wrap"><span class="bwp-card-badge version">WP '+esc(currentWpInstance.version)+'</span><span class="bwp-card-badge '+(currentWpInstance.alive?"status-active":"status-inactive")+'"><span style="width:5px;height:5px;border-radius:50%;background:currentColor;display:inline-block"></span> '+(currentWpInstance.alive?"Active":"Inactive")+'</span>'+(totalUpdates>0?'<span class="bwp-card-badge updates">'+totalUpdates+' update'+(totalUpdates>1?"s":"")+'</span>':'')+(currentWpInstance.maintenance?'<span class="bwp-card-badge" id="bwpBadgeMaint" style="background:rgba(217,119,6,.08);color:#d97706">Maintenance</span>':'<span class="bwp-card-badge" id="bwpBadgeMaint" style="background:rgba(217,119,6,.08);color:#d97706;display:none">Maintenance</span>')+(currentWpInstance.infected?'<span class="bwp-card-badge" style="background:rgba(239,68,68,.08);color:#ef4444">Infected</span>':'')+'</div></div></div>';html+='<div class="bwp-detail-tabs"><button type="button" class="bwp-tab active" data-tab="overview">Overview</button><button type="button" class="bwp-tab" data-tab="plugins">Plugins</button><button type="button" class="bwp-tab" data-tab="themes">Themes</button><button type="button" class="bwp-tab" data-tab="security">Security</button></div>';html+='<div class="bwp-detail-body" id="bwpDetailBody"><div class="bwp-tab-content active" id="bwpTabOverview"></div><div class="bwp-tab-content" id="bwpTabPlugins"><div class="bt-loading"><div class="bt-spinner"></div><span>Loading plugins...</span></div></div><div class="bwp-tab-content" id="bwpTabThemes"><div class="bt-loading"><div class="bt-spinner"></div><span>Loading themes...</span></div></div><div class="bwp-tab-content" id="bwpTabSecurity"><div class="bt-loading"><div class="bt-spinner"></div><span>Running security scan...</span></div></div></div></div></div>';wpPage.innerHTML=html;bwpRenderOverview(siteUrl);$("bwpDetailLogin").addEventListener("click",function(){bwpAutoLogin(id);});wpPage.querySelectorAll(".bwp-tab").forEach(function(tab){tab.addEventListener("click",function(){wpPage.querySelectorAll(".bwp-tab").forEach(function(t){t.classList.remove("active");});wpPage.querySelectorAll(".bwp-tab-content").forEach(function(c){c.classList.remove("active");});tab.classList.add("active");var tabName=tab.getAttribute("data-tab");var target=$("bwpTab"+tabName.charAt(0).toUpperCase()+tabName.slice(1));if(target){target.classList.add("active");if(!target.getAttribute("data-loaded")){if(tabName==="plugins") bwpLoadPlugins();else if(tabName==="themes") bwpLoadThemes();else if(tabName==="security") bwpLoadSecurity();}}});});$("bwpInlineBack").addEventListener("click",function(){currentWpInstance=null;wpPage.innerHTML="";buildWpPaneInto(wpPage);loadWpInstances();});bwpLoadPlugins();}
function bwpRenderOverview(siteUrl){var ovTab=$("bwpTabOverview");if(!ovTab||!currentWpInstance) return;var inst=currentWpInstance;var ovHtml='<div class="bwp-overview-hero"><div class="bwp-preview-col"><div class="bwp-preview-wrap"><div class="bwp-preview-bar"><div class="bwp-preview-dots"><span></span><span></span><span></span></div><div class="bwp-preview-url">'+esc(siteUrl)+'</div></div><div class="bwp-preview-frame-wrap" id="bwpScreenshotWrap" style="height:198px;display:flex;align-items:center;justify-content:center;background:var(--input-bg,#f3f4f6)"><div class="bt-spinner" style="width:20px;height:20px;border-width:2px"></div></div></div></div>';ovHtml+='<div class="bwp-overview-right"><div class="bwp-overview-grid" style="grid-template-columns:1fr 1fr"><div class="bwp-stat"><div class="bwp-stat-label">Status</div><div class="bwp-stat-value">'+(inst.alive?'<span style="color:#059669">Active</span>':'<span style="color:#ef4444">Inactive</span>')+'</div></div><div class="bwp-stat"><div class="bwp-stat-label">SSL</div><div class="bwp-stat-value">'+(inst.ssl?'<span style="color:#059669">Enabled</span>':'<span style="color:#d97706">Disabled</span>')+'</div></div><div class="bwp-stat"><div class="bwp-stat-label">Plugin Updates</div><div class="bwp-stat-value">'+(inst.pluginUpdates>0?'<span style="color:#0a5ed3">'+inst.pluginUpdates+' available</span>':'<span style="color:#059669">Up to date</span>')+'</div></div><div class="bwp-stat"><div class="bwp-stat-label">Theme Updates</div><div class="bwp-stat-value">'+(inst.themeUpdates>0?'<span style="color:#7c3aed">'+inst.themeUpdates+' available</span>':'<span style="color:#059669">Up to date</span>')+'</div></div><div class="bwp-stat"><div class="bwp-stat-label">Path</div><div class="bwp-stat-value" style="font-family:monospace;font-size:12px">'+esc(inst.path||"/")+'</div></div><div class="bwp-stat"><div class="bwp-stat-label">Version</div><div class="bwp-stat-value">WordPress '+esc(inst.version)+'</div></div></div>';if(inst.availableUpdate) ovHtml+='<div class="bwp-msg info" style="margin-top:12px"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="flex-shrink:0;vertical-align:middle;margin-right:6px"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>Core update available: WordPress '+esc(inst.availableUpdate)+' <button type="button" class="bwp-item-btn update" id="bwpUpdateCore" style="margin-left:8px">Update Core</button></div>';ovHtml+='</div></div>';/* Quick Toggles */var maintOn=inst.maintenance||false;var debugOn=inst.debug||false;var hotlinkOn=inst.hotlinkProtection||false;ovHtml+='<div style="margin-top:20px"><div style="font-size:13px;font-weight:600;color:var(--heading-color,#111827);margin-bottom:10px">Quick Toggles</div><div class="bwp-tools-grid">';ovHtml+='<div class="bwp-tool-card"><div class="bwp-tool-icon" style="background:rgba(217,119,6,.08);color:#d97706"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg></div><div class="bwp-tool-info"><p class="bwp-tool-name">Maintenance Mode</p><p class="bwp-tool-desc" id="bwpMaintDesc">'+(maintOn?'Site is in maintenance mode':'Show maintenance page to visitors')+'</p></div><div class="bwp-tool-toggle"><button type="button" class="bwp-toggle-btn'+(maintOn?" on":"")+'" id="bwpToggleMaint" data-on="'+maintOn+'"><span class="bwp-toggle-knob"></span></button></div></div>';ovHtml+='<div class="bwp-tool-card"><div class="bwp-tool-icon" style="background:rgba(99,102,241,.08);color:#6366f1"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z"/><path d="M12 6v6l4 2"/></svg></div><div class="bwp-tool-info"><p class="bwp-tool-name">Debug Mode</p><p class="bwp-tool-desc" id="bwpDebugDesc">'+(debugOn?'WP_DEBUG is enabled':'Enable WordPress debug logging')+'</p></div><div class="bwp-tool-toggle"><button type="button" class="bwp-toggle-btn'+(debugOn?" on":"")+'" id="bwpToggleDebug" data-on="'+debugOn+'"><span class="bwp-toggle-knob"></span></button></div></div>';ovHtml+='<div class="bwp-tool-card"><div class="bwp-tool-icon" style="background:rgba(236,72,153,.08);color:#ec4899"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg></div><div class="bwp-tool-info"><p class="bwp-tool-name">Hotlink Protection</p><p class="bwp-tool-desc" id="bwpHotlinkDesc">'+(hotlinkOn?'Hotlink protection is active':'Prevent other sites from embedding your images')+'</p></div><div class="bwp-tool-toggle"><button type="button" class="bwp-toggle-btn'+(hotlinkOn?" on":"")+'" id="bwpToggleHotlink" data-on="'+hotlinkOn+'"><span class="bwp-toggle-knob"></span></button></div></div>';ovHtml+='<div class="bwp-tool-card"><div class="bwp-tool-icon" style="background:rgba(10,94,211,.08);color:#0a5ed3"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg></div><div class="bwp-tool-info"><p class="bwp-tool-name">Check for Updates</p><p class="bwp-tool-desc">Scan for available core, plugin, and theme updates</p></div><div class="bwp-tool-toggle"><button type="button" class="bwp-item-btn update" id="bwpCheckUpdates" style="padding:6px 14px">Check Now</button></div></div>';ovHtml+='</div></div>';/* Admin Accounts */ovHtml+='<div style="margin-top:20px"><div style="font-size:13px;font-weight:600;color:var(--heading-color,#111827);margin-bottom:10px">Admin Accounts</div><div id="bwpAdminAccounts"><div class="bt-loading" style="padding:12px"><div class="bt-spinner" style="width:16px;height:16px;border-width:2px"></div><span style="font-size:12px">Loading accounts...</span></div></div></div>';ovTab.innerHTML=ovHtml;/* Load screenshot */bwpLoadScreenshot(siteUrl);/* Load admin accounts */bwpLoadAccounts();/* Bind core update */var coreBtn=$("bwpUpdateCore");if(coreBtn) coreBtn.addEventListener("click",function(){btnLoad(this,"Updating...");var self=this;var tid=btProgress.add("Update WordPress Core","Updating core...");wpPost({action:"wp_update",instance_id:inst.id,type:"core"},function(r){if(r.success){btProgress.update(tid,"success","Core update initiated");fmToast("Core update initiated","success");self.disabled=true;self.textContent="Updating...";}else{btProgress.update(tid,"error",r.message||"Failed");btnDone(self);fmToast(r.message||"Update failed","error");}});});/* Bind toggles */bwpBindToggle("bwpToggleMaint","wp_maintenance","enable","maintenance","bwpMaintDesc","bwpBadgeMaint","Site is in maintenance mode","Show maintenance page to visitors");bwpBindToggle("bwpToggleDebug","wp_debug_toggle","enable","debug","bwpDebugDesc",null,"WP_DEBUG is enabled","Enable WordPress debug logging");bwpBindToggle("bwpToggleHotlink","wp_toggle_hotlink","enable","hotlinkProtection","bwpHotlinkDesc",null,"Hotlink protection is active","Prevent other sites from embedding your images");var checkBtn=$("bwpCheckUpdates");if(checkBtn) checkBtn.addEventListener("click",function(){btnLoad(this,"Checking...");var self=this;var tid=btProgress.add("Check for updates","Scanning...");wpPost({action:"wp_check_for_updates",instance_id:inst.id},function(r){btnDone(self);if(r.success){btProgress.update(tid,"success","Update check initiated");fmToast("Update check initiated. Refresh in a moment to see results.","success");}else{btProgress.update(tid,"error",r.message||"Failed");fmToast(r.message||"Failed","error");}});});}
function bwpBindToggle(btnId,action,param,prop,descId,badgeId,onDesc,offDesc){var btn=$(btnId);if(!btn||!currentWpInstance) return;btn.addEventListener("click",function(){var isOn=this.getAttribute("data-on")==="true";var newState=!isOn;var self=this;self.disabled=true;var label=prop.replace(/([A-Z])/g," $1").toLowerCase();var tid=btProgress.add((newState?"Enable":"Disable")+" "+label,"Toggling...");var d={action:action,instance_id:currentWpInstance.id};d[param]=newState?"1":"0";wpPost(d,function(r){self.disabled=false;if(r.success){btProgress.update(tid,"success",r.message||"Done");fmToast(r.message||"Done","success");self.setAttribute("data-on",String(newState));if(newState) self.classList.add("on"); else self.classList.remove("on");currentWpInstance[prop]=newState;var desc=$(descId);if(desc) desc.textContent=newState?onDesc:offDesc;var badge=$(badgeId);if(badge) badge.style.display=newState?"":"none";}else{btProgress.update(tid,"error",r.message||"Failed");fmToast(r.message||"Failed","error");}});});}
...
function bwpVerifyUpdate(tid,instId,type,slug,onDone){
    var attempt=0;var maxAttempts=10;
    function poll(){
        attempt++;btProgress.setDetail(tid,"Verifying update... ("+attempt+"/"+maxAttempts+")");
        wpPost({action:"wp_check_update_status",instance_id:instId,type:type,slug:slug},function(r){
            if(r.success&&r.updated){btProgress.update(tid,"success","Updated to latest version");if(onDone) onDone(true);}
            else if(attempt>=maxAttempts){btProgress.update(tid,"success","Update sent \u2014 could not confirm");if(onDone) onDone(true);}
...
'.bt-addon-card{background:var(--card-bg,#fff);border:1px solid var(--border-color,#e5e7eb);border-radius:12px;padding:20px;transition:box-shadow .2s,border-color .2s}',
...
'.bt-addon-card-title{font-size:15px;font-weight:600;color:var(--heading-color,#111827);line-height:1.3}',
'.bt-addon-card-desc{font-size:13px;color:var(--text-muted,#6b7280);line-height:1.6;margin-bottom:14px}',
...
'.bt-addon-card-desc strong{color:var(--heading-color,#111827)}',
'.bt-addon-card-footer{display:flex;align-items:center;justify-content:space-between;gap:8px;padding-top:14px;border-top:1px solid var(--border-color,#e5e7eb)}',
'.bt-addon-card .bt-addon-card-price{font-size:15px;font-weight:600;color:var(--heading-color,#111827)}',
...
'.bt-addons-page-btn.primary svg{stroke:#fff}',
'.bt-addons-page-btn.outline{background:var(--card-bg,#fff);border:1px solid var(--border-color,#d1d5db);color:var(--heading-color,#374151)}',
'.bt-addons-page-btn.outline:hover{border-color:#0a5ed3;color:#0a5ed3;background:rgba(10,94,211,.04);text-decoration:none}',
'.bt-addons-section-wrap{margin-bottom:28px}',
'.bt-addons-section-title{font-size:16px;font-weight:600;color:var(--heading-color,#111827);margin-bottom:4px}',
'.bt-addons-section-sub{font-size:13px;color:var(--text-muted,#6b7280);margin-bottom:0}',
'.bt-addons-empty{text-align:center;padding:40px 20px;color:var(--text-muted,#9ca3af)}',
'.bt-addons-empty svg{margin-bottom:12px;opacity:.4}',
'.bt-addons-loading{text-align:center;padding:60px 20px;color:var(--text-muted,#9ca3af)}',
'.bt-config-option{background:var(--input-bg,#f8fafc);border:1px solid var(--border-color,#e5e7eb);border-radius:10px;padding:16px;margin-bottom:12px}',
'.bt-config-option-name{font-size:14px;font-weight:600;color:var(--heading-color,#111827);margin-bottom:8px}',
'.bt-config-option-current{font-size:13px;color:var(--text-muted,#6b7280)}',
'.bt-config-option-current strong{color:var(--heading-color,#111827)}',
/* Dark mode for addons page */
'[data-theme="dark"] .bt-addon-card,.dark-mode .bt-addon-card{background:var(--card-bg,#1f2937);border-color:var(--border-color,#374151)}',
'[data-theme="dark"] .bt-addon-card:hover,.dark-mode .bt-addon-card:hover{border-color:#4b5563;box-shadow:0 4px 12px rgba(0,0,0,.2)}',
'[data-theme="dark"] .bt-addon-card-title,.dark-mode .bt-addon-card-title{color:var(--heading-color,#e5e7eb)}',
'[data-theme="dark"] .bt-addon-card-desc,.dark-mode .bt-addon-card-desc{color:var(--text-muted,#9ca3af)}',
'[data-theme="dark"] .bt-addon-card-desc strong,.dark-mode .bt-addon-card-desc strong{color:var(--heading-color,#e5e7eb)}',
'[data-theme="dark"] .bt-addon-card-footer,.dark-mode .bt-addon-card-footer{border-top-color:var(--border-color,#374151)}',
'[data-theme="dark"] .bt-addon-card .bt-addon-card-price,.dark-mode .bt-addon-card .bt-addon-card-price{color:var(--heading-color,#e5e7eb)}',
'[data-theme="dark"] .bt-config-option,.dark-mode .bt-config-option{background:var(--input-bg,#111827);border-color:var(--border-color,#374151)}',
'[data-theme="dark"] .bt-addons-page-btn.outline,.dark-mode .bt-addons-page-btn.outline{background:var(--card-bg,#1f2937);border-color:var(--border-color,#4b5563);color:var(--heading-color,#d1d5db)}',
...
function injectStyles11(){
    ...
    s.textContent=[
'.bt-progress-box{position:fixed;bottom:24px;right:24px;z-index:100005;width:340px;max-height:420px;display:flex;flex-direction:column;border-radius:14px;background:var(--card-bg,#fff);border:1px solid var(--border-color,#e5e7eb);box-shadow:0 12px 40px rgba(0,0,0,.15);overflow:hidden;animation:btProgressIn .25s ease;font-family:inherit}',
'@keyframes btProgressIn{from{opacity:0;transform:translateY(16px) scale(.96)}to{opacity:1;transform:translateY(0) scale(1)}}',
'.bt-progress-head{display:flex;align-items:center;justify-content:space-between;padding:12px 16px;border-bottom:1px solid var(--border-color,#f3f4f6);flex-shrink:0;cursor:pointer;user-select:none}',
'.bt-progress-head-left{display:flex;align-items:center;gap:8px}',
'.bt-progress-head-icon{width:28px;height:28px;border-radius:8px;background:rgba(10,94,211,.08);display:flex;align-items:center;justify-content:center;color:#0a5ed3;flex-shrink:0}',
'.bt-progress-title{font-size:13px;font-weight:700;color:var(--heading-color,#111827);margin:0}',
'.bt-progress-subtitle{font-size:11px;color:var(--text-muted,#9ca3af);margin:0}',
'.bt-progress-head-actions{display:flex;gap:4px}',
'.bt-progress-head-btn{width:26px;height:26px;border-radius:6px;border:none;background:none;cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--text-muted,#9ca3af);transition:background .12s,color .12s}.bt-progress-head-btn:hover{background:var(--input-bg,#f3f4f6);color:var(--heading-color,#111827)}',
'.bt-progress-body{flex:1;overflow-y:auto;padding:6px 0}',
'.bt-progress-body.collapsed{display:none}',
'.bt-progress-item{display:flex;align-items:flex-start;gap:10px;padding:10px 16px;transition:background .12s}.bt-progress-item:hover{background:var(--input-bg,#fafbfc)}',
'.bt-progress-item+.bt-progress-item{border-top:1px solid var(--border-color,#f3f4f6)}',
'.bt-progress-item-icon{width:24px;height:24px;border-radius:50%;display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:1px}',
'.bt-progress-item-icon.running{background:rgba(10,94,211,.1);color:#0a5ed3}',
'.bt-progress-item-icon.success{background:rgba(5,150,105,.1);color:#059669}',
'.bt-progress-item-icon.error{background:rgba(239,68,68,.1);color:#ef4444}',
'.bt-progress-item-icon.pending{background:var(--input-bg,#f3f4f6);color:var(--text-muted,#9ca3af)}',
'.bt-progress-item-body{flex:1;min-width:0}',
'.bt-progress-item-label{font-size:12px;font-weight:600;color:var(--heading-color,#111827);margin:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}',
'.bt-progress-item-detail{font-size:11px;color:var(--text-muted,#9ca3af);margin:2px 0 0}',
'.bt-progress-item-bar{height:3px;background:var(--input-bg,#e5e7eb);border-radius:2px;margin-top:6px;overflow:hidden}',
'.bt-progress-item-fill{height:100%;border-radius:2px;transition:width .4s ease}',
'.bt-progress-item-fill.running{background:linear-gradient(90deg,#0a5ed3,#3b82f6);animation:btProgressPulse 1.5s ease infinite}',
'.bt-progress-item-fill.success{background:#059669}',
'.bt-progress-item-fill.error{background:#ef4444}',
'@keyframes btProgressPulse{0%,100%{opacity:1}50%{opacity:.6}}',
'.bt-progress-item-icon.running svg{animation:btProgressSpin .8s linear infinite}',
'@keyframes btProgressSpin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}',
'.bt-progress-clear{display:flex;justify-content:center;padding:8px 16px;border-top:1px solid var(--border-color,#f3f4f6);flex-shrink:0}',
'.bt-progress-clear-btn{font-size:11px;font-weight:600;color:var(--text-muted,#9ca3af);background:none;border:none;cursor:pointer;padding:4px 12px;border-radius:6px;transition:background .12s,color .12s}.bt-progress-clear-btn:hover{background:var(--input-bg,#f3f4f6);color:var(--heading-color,#111827)}',
'[data-theme="dark"] .bt-progress-box,.dark-mode .bt-progress-box{background:var(--card-bg,#1f2937);border-color:var(--border-color,#374151);box-shadow:0 12px 40px rgba(0,0,0,.4)}',
'[data-theme="dark"] .bt-progress-item:hover,.dark-mode .bt-progress-item:hover{background:rgba(255,255,255,.03)}',
'[data-theme="dark"] .bt-progress-head-btn:hover,.dark-mode .bt-progress-head-btn:hover{background:rgba(255,255,255,.06)}',
'@media(max-width:480px){.bt-progress-box{width:calc(100vw - 32px);right:16px;bottom:16px}}',
    ].join('\n');
    document.head.appendChild(s);
}
...
var btProgress=(function(){
    ...
    var svgSuccess='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg>';
    var svgError='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>';
    function ensureBox(){
        injectStyles11();
        if(boxEl&&boxEl.parentNode) return;
        boxEl=document.createElement("div");boxEl.className="bt-progress-box";boxEl.id="bt-progress-box";
        boxEl.innerHTML='<div class="bt-progress-head" id="btProgressHead"><div class="bt-progress-head-left"><div class="bt-progress-head-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg></div><div><p class="bt-progress-title">Task Progress</p><p class="bt-progress-subtitle" id="btProgressSubtitle">0 tasks</p></div></div><div class="bt-progress-head-actions"><button type="button" class="bt-progress-head-btn" id="btProgressToggle" title="Collapse"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"/></svg></button><button type="button" class="bt-progress-head-btn" id="btProgressClose" title="Close"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button></div></div><div class="bt-progress-body" id="btProgressBody"></div><div class="bt-progress-clear" id="btProgressClear" style="display:none"><button type="button" class="bt-progress-clear-btn" id="btProgressClearBtn">Clear completed</button></div>';
        document.body.appendChild(boxEl);
        bodyEl=document.getElementById("btProgressBody");
        document.getElementById("btProgressToggle").addEventListener("click",function(e){e.stopPropagation();collapsed=!collapsed;bodyEl.classList.toggle("collapsed",collapsed);var clearEl=document.getElementById("btProgressClear");if(clearEl) clearEl.style.display=collapsed?"none":"";this.querySelector("svg").style.transform=collapsed?"rotate(180deg)":"";});
        document.getElementById("btProgressClose").addEventListener("click",function(e){e.stopPropagation();doClose();});
        document.getElementById("btProgressHead").addEventListener("click",function(){if(collapsed){collapsed=false;bodyEl.classList.remove("collapsed");var clearEl=document.getElementById("btProgressClear");if(clearEl) clearEl.style.display="";var togBtn=document.getElementById("btProgressToggle");if(togBtn) togBtn.querySelector("svg").style.transform="";}});
        document.getElementById("btProgressClearBtn").addEventListener("click",function(){clearDone();});
        if(hideTimer){clearTimeout(hideTimer);hideTimer=null;}
    }
    function doClose(){
        if(boxEl&&boxEl.parentNode){boxEl.style.opacity="0";boxEl.style.transform="translateY(16px)";boxEl.style.transition="opacity .2s,transform .2s";setTimeout(function(){if(boxEl&&boxEl.parentNode) boxEl.parentNode.removeChild(boxEl);boxEl=null;bodyEl=null;},200);}
        tasks={};taskOrder=[];
    }
    function updateSub(){
        var sub=document.getElementById("btProgressSubtitle");if(!sub) return;
        var running=0,done=0,failed=0;
        taskOrder.forEach(function(id){var t=tasks[id];if(!t) return;if(t.status==="running") running++;else if(t.status==="success") done++;else if(t.status==="error") failed++;});
        var parts=[];if(running>0) parts.push(running+" running");if(done>0) parts.push(done+" done");if(failed>0) parts.push(failed+" failed");
        sub.textContent=parts.length?parts.join(" \u00b7 "):(taskOrder.length+" task"+(taskOrder.length!==1?"s":""));
    }
    function renderItem(id){
        var t=tasks[id];if(!t||!bodyEl) return;
        var el=document.getElementById("btprog-"+id);
        var iconSvg=t.status==="running"?svgRunning:t.status==="success"?svgSuccess:svgError;
        var pct=t.status==="running"?(t.progress||30):100;
        var html='<div class="bt-progress-item-icon '+esc(t.status)+'">'+iconSvg+'</div><div class="bt-progress-item-body"><p class="bt-progress-item-label">'+esc(t.label)+'</p><p class="bt-progress-item-detail">'+esc(t.detail||"")+'</p>';
        if(t.status==="running") html+='<div class="bt-progress-item-bar"><div class="bt-progress-item-fill running" style="width:'+pct+'%"></div></div>';
        html+='</div>';
        if(!el){el=document.createElement("div");el.className="bt-progress-item";el.id="btprog-"+id;bodyEl.appendChild(el);}
        el.innerHTML=html;
    }
    function showClearBtn(){
        var clearEl=document.getElementById("btProgressClear");if(!clearEl) return;
        var hasDone=false;taskOrder.forEach(function(id){var t=tasks[id];if(t&&(t.status==="success"||t.status==="error")) hasDone=true;});
        clearEl.style.display=(hasDone&&!collapsed)?"":"none";
    }
    function clearDone(){
        var keep=[];
        taskOrder.forEach(function(id){var t=tasks[id];if(t&&t.status!=="success"&&t.status!=="error"){keep.push(id);}else{delete tasks[id];var el=document.getElementById("btprog-"+id);if(el&&el.parentNode) el.parentNode.removeChild(el);}});
        taskOrder=keep;updateSub();showClearBtn();
        if(!taskOrder.length) doClose();
    }
    function scheduleAutoHide(){
        if(hideTimer){clearTimeout(hideTimer);hideTimer=null;}
        var allDone=true;taskOrder.forEach(function(id){var t=tasks[id];if(t&&t.status==="running") allDone=false;});
        if(allDone&&taskOrder.length>0){hideTimer=setTimeout(function(){var hasErr=false;taskOrder.forEach(function(id){var t=tasks[id];if(t&&t.status==="error") hasErr=true;});if(!hasErr) doClose();},5000);}
    }
    return {
        add:function(label,detail){
            var id=nextId++;tasks[id]={label:label,detail:detail||"Starting...",status:"running",progress:15};taskOrder.push(id);
            ensureBox();
            if(collapsed){collapsed=false;if(bodyEl) bodyEl.classList.remove("collapsed");var togBtn=document.getElementById("btProgressToggle");if(togBtn) togBtn.querySelector("svg").style.transform="";}
            renderItem(id);updateSub();showClearBtn();
            var progTimer=setInterval(function(){var t=tasks[id];if(!t||t.status!=="running"){clearInterval(progTimer);return;}t.progress=Math.min(t.progress+Math.random()*12,90);renderItem(id);},800);
            tasks[id]._progTimer=progTimer;
            return id;
        },
        update:function(id,status,detail){
            var t=tasks[id];if(!t) return;
            if(t._progTimer){clearInterval(t._progTimer);delete t._progTimer;}
            t.status=status;if(detail!==undefined) t.detail=detail;else if(status==="success") t.detail="Completed";else if(status==="error") t.detail=t.detail||"Failed";
            t.progress=100;renderItem(id);updateSub();showClearBtn();scheduleAutoHide();
        },
        setDetail:function(id,detail){
            var t=tasks[id];if(!t) return;
            t.detail=detail;renderItem(id);
        },
        run:function(label,ajaxFn,data,opts){
            opts=opts||{};var id=this.add(label,opts.detail||"Processing...");var self=this;
            ajaxFn(data,function(r){
                if(r.success) self.update(id,"success",opts.successMsg||(r.message||"Completed"));
                else self.update(id,"error",opts.errorMsg||(r.message||"Failed"));
                if(opts.callback) opts.callback(r,id);
            });
            return id;
        }
    };
...
function buildAddonsPageInto(container){
    ...
    function sanitizeDesc(html){
        ...
        var dangerous=tmp.querySelectorAll('script,style,iframe,object,embed,form,input,textarea,select,button,link,meta');
        ...
        tmp.querySelectorAll('*').forEach(function(el){
            ...
            for(var j=attrs.length-1;j>=0;j--){
                ...
                if(name.indexOf('on')===0||name==='style'&&attrs[j].value.indexOf('expression')!==-1) el.removeAttribute(name);
            }
            /* Remove javascript: hrefs */
            if(el.tagName==='A'&&el.href&&el.href.toLowerCase().indexOf('javascript:')===0) el.removeAttribute('href');
        });
    ...
    post({action:'get_addons'},function(r){
        ...
        if(activeAddons.length>0){
            html+='<div class="bt-addons-section-wrap"><div class="bt-addons-section-title">Active Addons</div><p class="bt-addons-section-sub">Addons currently active on this service</p>';
            html+='<div class="bt-addons-grid">';
            activeAddons.forEach(function(a){
                var statusClass=(a.status||'').toLowerCase().replace(/[^a-z]/g,'');
                ...
                var safeDesc=sanitizeDesc(a.description);
                if(safeDesc) html+='<div class="bt-addon-card-desc">'+safeDesc+'</div>';
                html+='<div class="bt-addon-card-footer">';
                html+='<div><span class="bt-addon-badge '+statusClass+'">'+esc(a.status)+'</span></div>';
                html+='<div style="text-align:right">';
                if(a.price) html+='<div class="bt-addon-card-price">'+esc(a.price)+'</div>';
                if(a.nextDue&&a.nextDue!=='0000-00-00') html+='<div style="font-size:12px;color:var(--text-muted,#9ca3af)">Due: '+esc(a.nextDue)+'</div>';
                html+='</div></div></div>';
            });
            html+='</div></div>';
        }
        ...
        if(notYetActive.length>0){
            html+='<div class="bt-addons-section-wrap"><div class="bt-addons-section-title">Available Addons</div><p class="bt-addons-section-sub">Additional services you can add to your hosting</p>';
            html+='<div class="bt-addons-grid">';
            notYetActive.forEach(function(a){
                ...
                var safeDesc2=sanitizeDesc(a.description);
                if(safeDesc2) html+='<div class="bt-addon-card-desc">'+safeDesc2+'</div>';
                html+='<div class="bt-addon-card-footer">';
                if(a.price) html+='<div class="bt-addon-card-price">'+esc(a.price)+'</div>';
                else html+='<div></div>';
                html+='<a href="cart.php?a=add&aid='+a.id+'&serviceid='+C.serviceId+'" class="bt-addons-page-btn primary"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg> Order</a>';
                html+='</div></div>';
            });
            html+='</div></div>';
        }
        ...
        if(configOptions.length>0){
            html+='<div class="bt-addons-section-wrap"><div class="bt-addons-section-title">Configurable Options</div><p class="bt-addons-section-sub">Current configuration for this service</p><div style="margin-top:12px">';
            ...
            html+='</div><div style="margin-top:12px"><a href="upgrade.php?type=configoptions&id='+C.serviceId+'" class="bt-addons-page-btn outline"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg> Change Options</a></div>';
            html+='</div>';
        }
        ...
        html+='<div style="margin-top:20px;padding-top:16px;border-top:1px solid var(--border-color,#e5e7eb);display:flex;gap:10px;flex-wrap:wrap">';
        html+='<a href="upgrade.php?type=package&id='+C.serviceId+'" class="bt-addons-page-btn outline"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg> Upgrade / Downgrade Plan</a>';
        html+='</div>';
...
function injectStyles13(){
    ...
    s.textContent=""
    /* Email tabs */
    +".bt-email-tabs{display:flex;gap:0;border-bottom:2px solid var(--border-color,#e5e7eb);margin-bottom:16px}"
    +".bt-email-tab{display:inline-flex;align-items:center;gap:6px;padding:10px 18px;font-size:13px;font-weight:600;color:var(--text-muted,#6b7280);background:none;border:none;cursor:pointer;border-bottom:2px solid transparent;margin-bottom:-2px;transition:all .15s}"
    +".bt-email-tab:hover{color:var(--heading-color,#374151)}"
    +".bt-email-tab.active{color:#d97706;border-bottom-color:#d97706}"
    +".bt-email-tab svg{flex-shrink:0}"
    /* Connect wrap */
    ...
    +".bt-email-app-links{display:flex;gap:10px;flex-shrink:0;flex-wrap:wrap}"
    +".bt-email-app-store-btn{display:inline-flex;align-items:center;gap:10px;padding:10px 18px;border-radius:10px;text-decoration:none;font-size:13px;font-weight:600;transition:all .15s;border:1px solid rgba(255,255,255,.2)}"
    +".bt-email-app-store-btn.android{background:rgba(255,255,255,.1);color:#fff}"
    +".bt-email-app-store-btn.android:hover{background:rgba(255,255,255,.18);text-decoration:none;color:#fff}"
    +".bt-email-app-store-btn.ios{background:rgba(255,255,255,.1);color:#fff}"
    +".bt-email-app-store-btn.ios:hover{background:rgba(255,255,255,.18);text-decoration:none;color:#fff}"
    +".bt-email-app-store-btn span{display:flex;flex-direction:column;line-height:1.2}"
    ...
    +".bt-email-config-header{display:flex;align-items:center;gap:12px;padding:16px 18px;border-bottom:1px solid var(--border-color,#f3f4f6)}"
    +".bt-email-config-header h5{margin:0;font-size:14px;font-weight:700;color:var(--heading-color,#111827)}"
    +".bt-email-config-header p{margin:0;font-size:11px;color:var(--text-muted,#9ca3af)}"
    +".bt-email-config-icon{width:38px;height:38px;border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0}"
    +".bt-email-config-icon.imap{background:rgba(10,94,211,.08);color:#0a5ed3}"
    +".bt-email-config-icon.pop3{background:rgba(124,58,237,.08);color:#7c3aed}"
    +".bt-email-config-icon.smtp{background:rgba(5,150,105,.08);color:#059669}"
    +".bt-email-config-icon.guides{background:rgba(217,119,6,.08);color:#d97706}"
    +".bt-email-config-body{padding:4px 0}"
    +".bt-email-config-row{display:flex;align-items:center;justify-content:space-between;padding:9px 18px;gap:12px}"
    +".bt-email-config-row:not(:last-child){border-bottom:1px solid var(--border-color,#f8f9fa)}"
    +".bt-email-config-label{font-size:12px;font-weight:500;color:var(--text-muted,#6b7280);white-space:nowrap}"
    +".bt-email-config-val{font-size:12px;font-weight:600;color:var(--heading-color,#111827);font-family:SFMono-Regular,Consolas,monospace;text-align:right;display:inline-flex;align-items:center;gap:5px}"
    +".bt-email-config-val.copyable{cursor:pointer;padding:3px 8px;border-radius:6px;transition:background .12s}"
    +".bt-email-config-val.copyable:hover{background:rgba(10,94,211,.06);color:#0a5ed3}"
    +".bt-email-config-val.copyable svg{opacity:.4;transition:opacity .12s}"
    +".bt-email-config-val.copyable:hover svg{opacity:1}"
    /* Client grid */
    +".bt-email-clients-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:10px;padding:4px 0}"
    +".bt-email-client-item{display:flex;flex-direction:column;align-items:center;gap:6px;padding:14px 10px;border-radius:10px;border:1px solid var(--border-color,#f3f4f6);transition:all .15s;text-align:center;text-decoration:none;cursor:pointer}"
    +".bt-email-client-item:hover{border-color:#0a5ed3;background:rgba(10,94,211,.03);text-decoration:none;box-shadow:0 2px 8px rgba(10,94,211,.08)}"
    +".bt-email-client-icon{width:40px;height:40px;border-radius:10px;display:flex;align-items:center;justify-content:center}"
    +".bt-email-client-item span{font-size:12px;font-weight:600;color:var(--heading-color,#111827)}"
    +".bt-email-client-item small{font-size:10px;color:var(--text-muted,#9ca3af);font-weight:500}"
    /* Setup note */
    +".bt-email-setup-note{display:flex;align-items:flex-start;gap:8px;padding:12px 14px;margin-top:8px;border-radius:8px;background:rgba(217,119,6,.05);border:1px solid rgba(217,119,6,.12);font-size:12px;color:var(--text-muted,#6b7280);line-height:1.6}"
...
function renderAnalytics(results){
    ...
    html+='<a class="bt-analytics-stat-card" href="#" onclick="btOpenCpanelPage(\'bandwidth\',this);return false;"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg> Bandwidth</a>';
    html+='<a class="bt-analytics-stat-card" href="#" onclick="btOpenCpanelPage(\'awstats\',this);return false;"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 20V10"/><path d="M12 20V4"/><path d="M6 20v-6"/></svg> AWStats</a>';
    html+='</div></div>';
    body.innerHTML=html;
    setTimeout(function(){
