...
console.log("[BT] bt_client.js loaded successfully, version 3.10.58");
...
/* WP card list layout */
'.bwp-card-list{display:flex;flex-direction:column;gap:12px;padding:14px 16px}',
'.bwp-card-item{display:flex;gap:16px;border:1px solid var(--border-color,#e5e7eb);border-radius:12px;overflow:hidden;transition:border-color .15s,box-shadow .15s}.bwp-card-item:hover{border-color:rgba(10,94,211,.25);box-shadow:0 2px 8px rgba(0,0,0,.04)}',
'.bwp-card-preview{width:200px;min-width:200px;flex-shrink:0;background:var(--input-bg,#f3f4f6);display:flex;flex-direction:column;border-right:1px solid var(--border-color,#e5e7eb)}',
'.bwp-card-preview-bar{display:flex;align-items:center;gap:6px;padding:6px 10px;background:var(--input-bg,#f9fafb);border-bottom:1px solid var(--border-color,#e5e7eb)}',
'.bwp-card-preview-frame{flex:1;overflow:hidden;position:relative;min-height:120px}',
'.bwp-card-body{flex:1;min-width:0;padding:14px 16px;display:flex;flex-direction:column;justify-content:center;gap:10px}',
'.bwp-card-title-row{display:flex;align-items:center;gap:10px}',
'.bwp-card-title-icon{width:36px;height:36px;border-radius:9px;background:rgba(33,117,208,.08);display:flex;align-items:center;justify-content:center;color:#2175d0;flex-shrink:0}',
'.bwp-card-title-info{flex:1;min-width:0}',
'.bwp-card-domain{margin:0;font-size:14px;font-weight:600;color:var(--heading-color,#111827);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}',
'.bwp-card-actions{display:flex;gap:6px;flex-wrap:wrap}',
'@media(max-width:600px){.bwp-card-item{flex-direction:column}.bwp-card-preview{width:100%;min-width:0;border-right:none;border-bottom:1px solid var(--border-color,#e5e7eb)}.bwp-card-preview-frame{min-height:100px}}',
/* WP inline detail */
'.bwp-inline-detail{padding:20px 0}',
'.bwp-inline-detail .bwp-detail-head{border-radius:0;border-bottom:1px solid var(--border-color,#f3f4f6)}',
...
function loadWpInstances(){var list=$("bt-wp-list");if(!list) return;list.innerHTML='<div class="bt-loading"><div class="bt-spinner"></div><span>Loading WordPress installations...</span></div>';wpPost({action:"get_wp_instances"},function(r){if(!r.success){list.innerHTML='<div class="bt-empty"><span>'+(r.message||"Failed to load")+'</span></div>';return;}wpInstances=r.instances||[];var countEl=document.querySelector(".bt-wp-count");if(countEl) countEl.textContent=wpInstances.length+" site"+(wpInstances.length!==1?"s":"");if(!wpInstances.length){list.innerHTML='<div class="bt-empty">'+wpSvg32+'<span>No WordPress installations found</span></div>';return;}var html='<div class="bwp-card-list">';wpInstances.forEach(function(inst){var statusCls=inst.alive?"active":"inactive";var statusTxt=inst.alive?"Active":"Inactive";var siteUrl=inst.site_url||"";var meta='<span>WP '+esc(inst.version)+'</span>';if(inst.pluginUpdates>0) meta+='<span style="color:#0a5ed3">'+inst.pluginUpdates+' plugin update'+(inst.pluginUpdates>1?"s":"")+'</span>';if(inst.themeUpdates>0) meta+='<span style="color:#7c3aed">'+inst.themeUpdates+' theme update'+(inst.themeUpdates>1?"s":"")+'</span>';if(inst.availableUpdate) meta+='<span style="color:#d97706">Core update: '+esc(inst.availableUpdate)+'</span>';html+='<div class="bwp-card-item" data-id="'+inst.id+'"><div class="bwp-card-preview"><div class="bwp-card-preview-bar"><div class="bwp-preview-dots"><span></span><span></span><span></span></div><div class="bwp-preview-url">'+esc(siteUrl)+'</div></div><div class="bwp-card-preview-frame"><iframe src="'+esc(siteUrl)+'" style="width:200%;height:200%;transform:scale(.5);transform-origin:0 0;border:none;pointer-events:none" loading="lazy" sandbox="allow-same-origin"></iframe></div></div><div class="bwp-card-body"><div class="bwp-card-title-row"><div class="bwp-card-title-icon">'+wpSvg20+'</div><div class="bwp-card-title-info"><p class="bwp-card-domain">'+esc(inst.displayTitle||inst.domain)+'</p><div class="bwp-site-meta"><span class="bwp-status-badge '+statusCls+'"><span style="width:6px;height:6px;border-radius:50%;background:currentColor;display:inline-block"></span> '+statusTxt+'</span>'+meta+'</div></div></div><div class="bwp-card-actions"><button type="button" class="bt-row-btn login" data-wpid="'+inst.id+'"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4"/><polyline points="10 17 15 12 10 7"/><line x1="15" y1="12" x2="3" y2="12"/></svg><span>Login</span></button><a href="'+esc(siteUrl)+'" target="_blank" class="bt-row-btn visit"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg><span>Visit</span></a><button type="button" class="bt-row-btn" data-wpdetail="'+inst.id+'" style="color:#0a5ed3"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg><span>Manage</span></button></div></div></div>';});html+='</div>';list.innerHTML=html;list.querySelectorAll(".bt-row-btn.login[data-wpid]").forEach(function(b){b.addEventListener("click",function(){bwpAutoLogin(parseInt(this.getAttribute("data-wpid")));});});list.querySelectorAll("[data-wpdetail]").forEach(function(b){b.addEventListener("click",function(){bwpOpenDetail(parseInt(this.getAttribute("data-wpdetail")));});});});}
