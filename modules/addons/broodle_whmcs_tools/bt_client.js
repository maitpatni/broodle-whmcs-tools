...
console.log("[BT] bt_client.js loaded successfully, version 3.10.76");
...
function bwpLoadAccounts(){var el=$("bwpAdminAccounts");if(!el||!currentWpInstance) return;wpPost({action:"wp_get_account",instance_id:currentWpInstance.id},function(r){if(!r.success){el.innerHTML='<div style="font-size:12px;color:var(--text-muted,#6b7280)">Could not load accounts</div>';return;}var acc=r.account||{};var logins=acc.availableLogins||[];if(!logins.length&&acc.currentLogin) logins=[acc.currentLogin];if(!logins.length){el.innerHTML='<div style="font-size:12px;color:var(--text-muted,#6b7280)">No admin accounts found</div>';return;}var html='<div style="display:flex;flex-direction:column;gap:6px">';logins.forEach(function(login){html+='<div class="bwp-tool-card" style="padding:10px 14px"><div class="bwp-tool-icon" style="background:rgba(10,94,211,.08);color:#0a5ed3;width:30px;height:30px;border-radius:7px"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div><div class="bwp-tool-info" style="flex:1"><p class="bwp-tool-name" style="font-size:13px">'+esc(login)+'</p>'+(login===acc.currentLogin&&acc.currentEmail?'<p class="bwp-tool-desc" style="font-size:11px">'+esc(acc.currentEmail)+'</p>':'')+'</div><div style="display:flex;gap:6px"><button type="button" class="bwp-item-btn" onclick="bwpChangePassword(\''+esc(login)+'\')"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg> Reset Password</button></div></div>';});html+='</div>';el.innerHTML=html;});}
window.bwpChangePassword=function(login){if(!currentWpInstance) return;var newPass=prompt("Enter new password for "+login+":");if(!newPass) return;var tid=btProgress.add("Reset password: "+login,"Changing password...");wpPost({action:"wp_change_password",instance_id:currentWpInstance.id,login:login,new_password:newPass},function(r){if(r.success){btProgress.update(tid,"success","Password changed");fmToast("Password changed for "+login,"success");}else{btProgress.update(tid,"error",r.message||"Failed");fmToast(r.message||"Failed to change password","error");}});};
