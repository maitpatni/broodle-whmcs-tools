...
console.log("[BT] bt_client.js loaded successfully, version 3.10.75");
...
window.bwpApplySecurity=function(measureId,btn){if(!currentWpInstance) return;btnLoad(btn,"Applying...");var tid=btProgress.add("Apply: "+measureId,"Applying security measure...");wpPost({action:"wp_security_apply",instance_id:currentWpInstance.id,measure_id:measureId},function(r){btnDone(btn);if(r.success){btProgress.update(tid,"success","Security measure applied");fmToast("Security measure applied","success");var el=$("bwpTabSecurity");if(el) el.removeAttribute("data-loaded");bwpLoadSecurity();}else{btProgress.update(tid,"error",r.message||"Failed to apply");fmToast(r.message||"Failed to apply","error");}});};
window.bwpRevertSecurity=function(measureId,btn){if(!currentWpInstance) return;btnLoad(btn,"Reverting...");var tid=btProgress.add("Revert: "+measureId,"Reverting security measure...");wpPost({action:"wp_security_revert",instance_id:currentWpInstance.id,measure_id:measureId},function(r){btnDone(btn);if(r.success){btProgress.update(tid,"success","Security measure reverted");fmToast("Security measure reverted","success");var el=$("bwpTabSecurity");if(el) el.removeAttribute("data-loaded");bwpLoadSecurity();}else{btProgress.update(tid,"error",r.message||"Failed to revert");fmToast(r.message||"Failed to revert","error");}});};
...
/* ─── Tools Tab: Maintenance, Debug, Hotlink, Indexing, Check Updates ─── */
function bwpLoadTools(){if(!currentWpInstance) return;var el=$("bwpTabTools");if(!el) return;el.innerHTML='<div class="bt-loading"><div class="bt-spinner"></div><span>Loading tools...</span></div>';var instId=currentWpInstance.id;var pending=3;var debugData=null;var maintData=null;var accountData=null;function done(){pending--;if(pending>0) return;var html='<div style="display:flex;flex-direction:column;gap:16px">';/* Quick toggles section */html+='<div style="font-size:13px;font-weight:600;color:var(--heading-color,#111827);margin-bottom:4px">Quick Toggles</div>';html+='<div class="bwp-tools-grid">';/* Maintenance toggle */var maintOn=currentWpInstance.maintenance||false;html+='<div class="bwp-tool-card"><div class="bwp-tool-icon" style="background:rgba(217,119,6,.08);color:#d97706"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg></div><div class="bwp-tool-info"><p class="bwp-tool-name">Maintenance Mode</p><p class="bwp-tool-desc">'+(maintOn?'Site is in maintenance mode':'Show maintenance page to visitors')+'</p></div><div class="bwp-tool-toggle"><button type="button" class="bwp-toggle-btn'+(maintOn?" on":"")+'" id="bwpToggleMaint" data-on="'+maintOn+'"><span class="bwp-toggle-knob"></span></button></div></div>';/* Debug toggle */var debugOn=currentWpInstance.debug||false;html+='<div class="bwp-tool-card"><div class="bwp-tool-icon" style="background:rgba(99,102,241,.08);color:#6366f1"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z"/><path d="M12 6v6l4 2"/></svg></div><div class="bwp-tool-info"><p class="bwp-tool-name">Debug Mode</p><p class="bwp-tool-desc">'+(debugOn?'WP_DEBUG is enabled':'Enable WordPress debug logging')+'</p>';if(debugData){html+='<div style="margin-top:6px;display:flex;gap:6px;flex-wrap:wrap">';var dkeys=["wpDebug","wpDebugLog","wpDebugDisplay","scriptDebug","saveQueries"];dkeys.forEach(function(k){var v=debugData[k];if(v!==undefined) html+='<span style="font-size:10px;padding:2px 6px;border-radius:4px;background:'+(v?'rgba(5,150,105,.08);color:#059669':'rgba(107,114,128,.08);color:#6b7280')+'">'+k+': '+(v?'on':'off')+'</span>';});html+='</div>';}html+='</div><div class="bwp-tool-toggle"><button type="button" class="bwp-toggle-btn'+(debugOn?" on":"")+'" id="bwpToggleDebug" data-on="'+debugOn+'"><span class="bwp-toggle-knob"></span></button></div></div>';/* Hotlink protection toggle */var hotlinkOn=currentWpInstance.hotlinkProtection||false;html+='<div class="bwp-tool-card"><div class="bwp-tool-icon" style="background:rgba(236,72,153,.08);color:#ec4899"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg></div><div class="bwp-tool-info"><p class="bwp-tool-name">Hotlink Protection</p><p class="bwp-tool-desc">'+(hotlinkOn?'Hotlink protection is active':'Prevent other sites from embedding your images')+'</p></div><div class="bwp-tool-toggle"><button type="button" class="bwp-toggle-btn'+(hotlinkOn?" on":"")+'" id="bwpToggleHotlink" data-on="'+hotlinkOn+'"><span class="bwp-toggle-knob"></span></button></div></div>';/* Check for updates */html+='<div class="bwp-tool-card"><div class="bwp-tool-icon" style="background:rgba(10,94,211,.08);color:#0a5ed3"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg></div><div class="bwp-tool-info"><p class="bwp-tool-name">Check for Updates</p><p class="bwp-tool-desc">Scan for available core, plugin, and theme updates</p></div><div class="bwp-tool-toggle"><button type="button" class="bwp-item-btn update" id="bwpCheckUpdates" style="padding:6px 14px">Check Now</button></div></div>';html+='</div>';/* Account info */if(accountData){html+='<div style="font-size:13px;font-weight:600;color:var(--heading-color,#111827);margin-top:8px;margin-bottom:4px">Admin Account</div>';html+='<div class="bwp-tools-grid">';html+='<div class="bwp-tool-card"><div class="bwp-tool-icon" style="background:rgba(10,94,211,.08);color:#0a5ed3"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div><div class="bwp-tool-info"><p class="bwp-tool-name">Admin Login</p><p class="bwp-tool-desc" style="font-family:monospace;font-size:12px">'+esc(accountData.currentLogin||"N/A")+'</p></div></div>';html+='<div class="bwp-tool-card"><div class="bwp-tool-icon" style="background:rgba(5,150,105,.08);color:#059669"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg></div><div class="bwp-tool-info"><p class="bwp-tool-name">Admin Email</p><p class="bwp-tool-desc" style="font-family:monospace;font-size:12px">'+esc(accountData.currentEmail||"N/A")+'</p></div></div>';html+='</div>';}html+='</div>';el.innerHTML=html;el.setAttribute("data-loaded","1");/* Bind toggles */var toggleMaint=$("bwpToggleMaint");if(toggleMaint) toggleMaint.addEventListener("click",function(){var isOn=this.getAttribute("data-on")==="true";var newState=!isOn;var self=this;self.disabled=true;var tid=btProgress.add((newState?"Enable":"Disable")+" maintenance mode","Toggling...");wpPost({action:"wp_maintenance",instance_id:instId,enable:newState?"1":"0"},function(r){self.disabled=false;if(r.success){btProgress.update(tid,"success",r.message);fmToast(r.message,"success");self.setAttribute("data-on",String(newState));if(newState) self.classList.add("on"); else self.classList.remove("on");currentWpInstance.maintenance=newState;}else{btProgress.update(tid,"error",r.message||"Failed");fmToast(r.message||"Failed","error");}});});var toggleDebug=$("bwpToggleDebug");if(toggleDebug) toggleDebug.addEventListener("click",function(){var isOn=this.getAttribute("data-on")==="true";var newState=!isOn;var self=this;self.disabled=true;var tid=btProgress.add((newState?"Enable":"Disable")+" debug mode","Toggling...");wpPost({action:"wp_debug_toggle",instance_id:instId,enable:newState?"1":"0"},function(r){self.disabled=false;if(r.success){btProgress.update(tid,"success",r.message);fmToast(r.message,"success");self.setAttribute("data-on",String(newState));if(newState) self.classList.add("on"); else self.classList.remove("on");currentWpInstance.debug=newState;}else{btProgress.update(tid,"error",r.message||"Failed");fmToast(r.message||"Failed","error");}});});var toggleHotlink=$("bwpToggleHotlink");if(toggleHotlink) toggleHotlink.addEventListener("click",function(){var isOn=this.getAttribute("data-on")==="true";var newState=!isOn;var self=this;self.disabled=true;var tid=btProgress.add((newState?"Enable":"Disable")+" hotlink protection","Toggling...");wpPost({action:"wp_toggle_hotlink",instance_id:instId,enable:newState?"1":"0"},function(r){self.disabled=false;if(r.success){btProgress.update(tid,"success",r.message);fmToast(r.message,"success");self.setAttribute("data-on",String(newState));if(newState) self.classList.add("on"); else self.classList.remove("on");currentWpInstance.hotlinkProtection=newState;}else{btProgress.update(tid,"error",r.message||"Failed");fmToast(r.message||"Failed","error");}});});var checkBtn=$("bwpCheckUpdates");if(checkBtn) checkBtn.addEventListener("click",function(){btnLoad(this,"Checking...");var self=this;var tid=btProgress.add("Check for updates","Scanning...");wpPost({action:"wp_check_for_updates",instance_id:instId},function(r){btnDone(self);if(r.success){btProgress.update(tid,"success","Update check initiated");fmToast("Update check initiated. Refresh in a moment to see results.","success");}else{btProgress.update(tid,"error",r.message||"Failed");fmToast(r.message||"Failed","error");}});});}/* Fetch debug settings */wpPost({action:"wp_get_debug_settings",instance_id:instId},function(r){debugData=r.success?r.settings:null;done();});/* Fetch maintenance settings */wpPost({action:"wp_get_maintenance_settings",instance_id:instId},function(r){maintData=r.success?r.settings:null;done();});/* Fetch account info */wpPost({action:"wp_get_account",instance_id:instId},function(r){accountData=r.success?r.account:null;done();});}
