...
console.log("[BT] bt_client.js loaded successfully, version 3.10.73");
...
window.bwpSecurityDebug=function(){if(!currentWpInstance){console.log("No WP instance selected");return;}console.log("Testing security endpoints for instance "+currentWpInstance.id+"...");wpPost({action:"wp_security_debug",instance_id:currentWpInstance.id},function(r){console.log("=== WP Toolkit Security API Debug ===");if(r.tests){r.tests.forEach(function(t,i){console.log("\n--- Test "+(i+1)+": "+t.endpoint+" ---");console.log("HTTP:",t.http,"Success:",t.success);if(t.body) console.log("Body:",JSON.stringify(t.body));if(t.data) console.log("Data:",JSON.stringify(t.data,null,2));if(t.raw) console.log("Raw:",t.raw);});}else{console.log("Response:",JSON.stringify(r,null,2));}});};
