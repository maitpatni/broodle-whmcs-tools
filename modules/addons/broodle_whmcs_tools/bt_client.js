...
function styleSidebarActions(){
    ...
    navs.forEach(function(nav){
        ...
        items.forEach(function(item){
            ...
            // Hide any existing forms (SSO) but keep them in DOM for click handlers
            item.querySelectorAll("form").forEach(function(f){f.style.display="none";});
            ...
            // Clear visible text nodes without removing child elements
            var childNodes=Array.prototype.slice.call(item.childNodes);
            childNodes.forEach(function(n){
                if(n.nodeType===3) n.textContent=""; // text nodes
            });
            // Hide any remaining visible text spans that aren't our label
            item.querySelectorAll("span:not(.bt-action-label span)").forEach(function(sp){
                if(!sp.classList.contains("bt-action-icon")&&!sp.classList.contains("bt-action-label")) sp.style.display="none";
            });
            item.insertBefore(labelDiv,item.firstChild);
            item.insertBefore(iconDiv,item.firstChild);
            // Force visible — Lagom2 SSO JS may set display:none during processing
