...
console.log("[BT] bt_client.js loaded successfully, version 3.10.69");
...
function startAutoSSL(btn){if(!btn)return;var origHtml=btn.innerHTML;btn.disabled=true;btn.innerHTML='<div class="bt-spinner" style="width:14px;height:14px;border-width:2px;display:inline-block;vertical-align:middle"></div> Running AutoSSL...';post({action:"start_autossl"},function(r){if(!r.success){btn.disabled=false;btn.innerHTML=origHtml;fmToast(r.message||"Failed to start AutoSSL","error");return;}pollAutoSSL(btn,origHtml);});}
...
window.bwpActivateTheme=function(slug,btn){if(!currentWpInstance) return;btnLoad(btn,"Activating...");wpPost({action:"wp_toggle_theme",instance_id:currentWpInstance.id,slug:slug},function(r){if(r.success){fmToast("Theme activated successfully","success");var el=$("bwpTabThemes");if(el) el.removeAttribute("data-loaded");bwpLoadThemes();}else{btnDone(btn);fmToast(r.message||"Failed","error");}});};
window.bwpUpdateTheme=function(slug,btn){if(!currentWpInstance) return;btnLoad(btn,"Updating...");wpPost({action:"wp_update",instance_id:currentWpInstance.id,type:"themes",slug:slug},function(r){if(r.success){if(r.verified){btn.disabled=true;btn.innerHTML='<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#059669" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg> Updated';btn.style.cssText="color:#059669;border-color:#6ee7b7;pointer-events:none";fmToast("Theme updated successfully","success");}else{btn.innerHTML="Verifying...";btn.style.color="#d97706";fmToast("Update initiated, verifying...","info");}setTimeout(function(){var el=$("bwpTabThemes");if(el) el.removeAttribute("data-loaded");bwpLoadThemes();},1500);}else{btnDone(btn);fmToast(r.message||"Update failed","error");}});};
