...
console.log("[BT] bt_client.js loaded successfully, version 3.10.71");
...
window.bwpUpdateTheme=function(slug,btn){if(!currentWpInstance) return;btnLoad(btn,"Updating...");var tid=btProgress.add("Update theme: "+slug,"Updating theme...");wpPost({action:"wp_update",instance_id:currentWpInstance.id,type:"themes",slug:slug},function(r){if(r.success){if(r.verified){btProgress.update(tid,"success","Updated to latest version");btn.disabled=true;btn.innerHTML='<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#059669" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg> Updated';btn.style.cssText="color:#059669;border-color:#6ee7b7;pointer-events:none";}else{btProgress.update(tid,"success","Update initiated");btn.innerHTML="Verifying...";btn.style.color="#d97706";}setTimeout(function(){var el=$("bwpTabThemes");if(el) el.removeAttribute("data-loaded");bwpLoadThemes();},1500);}else{btProgress.update(tid,"error",r.message||"Update failed");btnDone(btn);fmToast(r.message||"Update failed","error");}});};
